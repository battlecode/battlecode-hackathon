...
export class Game {
    ...
    private doThrow(entity: Entity, action: ThrowAction): FastError {
        ...
        if (isOutOfBound(targetLoc, this.initialState) || this.occupied.get(targetLoc.x, targetLoc.y)) {
        ...
        while (!isOutOfBound(targetLoc, this.initialState) && !this.occupied.get(targetLoc.x, targetLoc.y)) {
