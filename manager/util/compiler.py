...
languages = (
	...
	#   arguments.
	Language("Ada", BOT, "Battle.adb",
		"./Battle",
		["*.ali"],
		[(["*.adb"], ExternalCompiler(comp_args["Ada"][0])),
			(["Battle.ali"], ExternalCompiler(comp_args["Ada"][1])),
			(["Battle.ali"], ExternalCompiler(comp_args["Ada"][2]))]
	),
	Language("C", BOT, "Battle.c",
		"./Battle",
		["*.o", BOT],
		[(["*.c"], TargetCompiler(comp_args["C"][0], targets["C"])),
			(["*.o"], ExternalCompiler(comp_args["C"][1]))]
	),
	Language("C#", BOT +".exe", "Battle.cs",
		"mono Battle.exe",
		[BOT + ".exe"],
		[(["*.cs"], ExternalCompiler(comp_args["C#"][0]))]
	),
	Language("VB", BOT +".exe", "Battle.vb",
		"mono Battle.exe",
		[BOT + ".exe"],
		[(["*.vb"],
			ExternalCompiler(comp_args["VB"][0], out_files=['Battle.exe']))]
	),
	# These two C++ variants should be combined after the ants contest
	Language("C++", BOT, "Battle.cc",
		"./Battle",
		["*.o", BOT],
		[
			(["*.c", "*.cpp", "*.cc"],
	...
	Language("Clojure", BOT +".clj", "Battle.clj",
		"java -Xmx%sm -cp /usr/share/java/clojure.jar:. clojure.main Battle.clj" % (MEMORY_LIMIT,),
		[],
		[(["*.clj"], ChmodCompiler("Clojure"))]
	),
	Language("CoffeeScript", BOT +".coffee", "Battle.coffee",
		"coffee Battle.coffee",
		[],
		[(["*.coffee"], ChmodCompiler("CoffeeScript"))]
	),
	Language("D", BOT, "Battle.d",
		"./Battle",
		["*.o", BOT],
		[(["*.d"], ExternalCompiler(comp_args["D"][0]))]
	),
	Language("Dart", BOT +".dart", "Battle.dart",
		"frogsh Battle.dart",
		[],
	...
	Language("Go", BOT, "Battle.go",
		"./Battle",
		["*.8", "*.6", BOT],
		[(["*.go"], ExternalCompiler(comp_args["Go"][0], out_files=['_go_.6'])),
			([""], ExternalCompiler(comp_args["Go"][1], out_files=['_go_.6']))]
	),
	Language("Groovy", BOT +".jar", "Battle.groovy",
		"java -Xmx" + str(MEMORY_LIMIT) + "m -cp Battle.jar:/usr/share/groovy/embeddable/groovy-all-1.7.5.jar Battle",
		["*.class, *.jar"],
		[(["*.groovy"], ExternalCompiler(comp_args["Groovy"][0])),
		(["*.class"], ExternalCompiler(comp_args["Groovy"][1]))]
	),
	Language("Haskell", BOT, "Battle.hs",
		"./Battle +RTS -M" + str(MEMORY_LIMIT) + "m",
		[BOT],
		[([""], ExternalCompiler(comp_args["Haskell"][0]))]
	),
	Language("Java", BOT +".jar", "Battle.java",
		"java -Xmx" + str(MEMORY_LIMIT) + "m -jar Battle.jar",
		["*.class", "*.jar"],
		[(["*.java"], ExternalCompiler(comp_args["Java"][0])),
			(["*.class"], ExternalCompiler(comp_args["Java"][1]))]
	),
	Language("Javascript", BOT +".js", "Battle.js",
		"node Battle.js",
		[],
		[(["*.js"], ChmodCompiler("Javascript"))]
	),
	Language("Lisp", BOT, "Battle.lisp",
		"./Battle --dynamic-space-size " + str(MEMORY_LIMIT),
		[BOT],
		[([""], ExternalCompiler(comp_args["Lisp"][0]))]
	),
	Language("Lua", BOT +".lua", "Battle.lua",
		"luajit-2.0.0-beta5 Battle.lua",
		[],
		[(["*.lua"], ChmodCompiler("Lua"))]
	),
	Language("OCaml", BOT +".native", "Battle.ml",
		"./Battle.native",
		[BOT + ".native"],
		[([""], ExternalCompiler(comp_args["OCaml"][0]))]
	),
	Language("Octave", BOT + ".m", "Battle.m",
		"octave -qf Battle.m",
		[],
	...
	Language("Perl", BOT +".pl", "Battle.pl",
		"perl Battle.pl",
		[],
		[(["*.pl"], ChmodCompiler("Perl"))]
	),
	Language("PHP", BOT +".php", "Battle.php",
		"php Battle.php",
		[],
		[(["*.php"], ChmodCompiler("PHP"))]
	),
	Language("Python3", BOT +".py", "Battle.py",
		"python3 Battle.py",
		["*.pyc"],
	...
	Language("PyPy", BOT +".pypy", "Battle.pypy",
		"pypy Battle.pypy",
		["*.pyc"],
		[(["*.py"], ChmodCompiler("Python"))]
	),
	Language("Racket", BOT +".rkt", "Battle.rkt",
		"racket Battle.rkt",
		[],
		[(["*.rkt"], ChmodCompiler("Racket"))]
	),
	Language("Ruby", BOT +".rb", "Battle.rb",
		"ruby Battle.rb",
		[],
		[(["*.rb"], ChmodCompiler("Ruby"))]
	),
	Language("Scala", BOT +".scala", "Battle.scala",
		'scala -J-Xmx'+ str(MEMORY_LIMIT) +'m -howtorun:object Battle',
		["*.scala, *.jar"],
		[(["*.scala"], ExternalCompiler(comp_args["Scala"][0]))]
	),
	Language("Scheme", BOT +".ss", "Battle.ss",
		"./Battle",
		[],
		[(["*.ss"], ChmodCompiler("Scheme"))]
	),
	Language("Tcl", BOT +".tcl", "Battle.tcl",
		"tclsh8.5 Battle.tcl",
		[],
...
_LANG_NOT_FOUND = """Did not find a recognized Battle.* file.
...
def detect_language(bot_dir):
	...
	with CD(bot_dir):
		...
		if len(detected_langs) > 1:
			return None, ['Found multiple Battle.* files: \n'+
